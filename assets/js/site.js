...
(() => {
    ...
    sectionTitles.forEach((title) => {
        ...
        title.innerHTML = words
            .map((word, idx) => '<span class="title-word" style="--word-delay:' +
            idx * 58 +
            'ms">' +
            word +
            "</span>")
    ...
    function animateCounter(el, config) {
        if (!config || el.dataset.counted === "true")
            return;
        ...
        function tick(now) {
            ...
            const value = Math.round(counterConfig.from + (counterConfig.to - counterConfig.from) * eased);
        ...
        const observer = new IntersectionObserver((entries, obs) => {
            entries.forEach((entry) => {
                ...
                if (entry.target.classList.contains("impact-tile")) {
                    const valueEl = entry.target.querySelector(".impact-value");
                    if (valueEl) {
                        animateCounter(valueEl, parseCounterText(valueEl.textContent ?? ""));
                    }
                }
